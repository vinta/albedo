...
ifeq ($(platform),gcp)
	...
	--verbosity debug \
	--cluster albedo \
	--properties "^;^spark.albedo.checkpointDir=gs://albedo/spark-data/checkpoint;spark.albedo.dataDir=gs://albedo/spark-data;spark.driver.memory=6g;spark.executor.cores=5;spark.executor.instances=4;spark.executor.memory=21g;spark.logConf=true;spark.serializer=org.apache.spark.serializer.KryoSerializer" \
...
ifeq ($(platform),gcp)
	...
	--verbosity debug \
	--cluster albedo \
	--properties "^;^spark.albedo.checkpointDir=gs://albedo/spark-data/checkpoint;spark.albedo.dataDir=gs://albedo/spark-data;spark.driver.memory=6g;spark.executor.cores=5;spark.executor.instances=4;spark.executor.memory=21g;spark.jars.packages=com.hankcs:hanlp:portable-1.3.4;spark.logConf=true;spark.serializer=org.apache.spark.serializer.KryoSerializer" \
