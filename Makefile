...
ifeq ($(platform),gcp)
	...
	--verbosity debug \
	--cluster albedo \
	--properties "^;^spark.albedo.checkpointDir=gs://albedo/spark-data/checkpoint;spark.albedo.dataDir=gs://albedo/spark-data;spark.driver.maxResultSize=3g;spark.driver.memory=6g;spark.executor.cores=5;spark.executor.instances=4;spark.executor.memory=36g;spark.serializer=org.apache.spark.serializer.KryoSerializer" \
...
ifeq ($(platform),gcp)
	...
	--verbosity debug \
	--cluster albedo \
	--properties "^;^spark.albedo.checkpointDir=gs://albedo/spark-data/checkpoint;spark.albedo.dataDir=gs://albedo/spark-data;spark.driver.maxResultSize=3g;spark.driver.memory=6g;spark.executor.cores=5;spark.executor.instances=4;spark.executor.memory=36g;spark.jars.packages=com.hankcs:hanlp:portable-1.3.4;spark.serializer=org.apache.spark.serializer.KryoSerializer" \
